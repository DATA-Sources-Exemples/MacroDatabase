...
JSON_FILES = json.load(open("https://raw.githubusercontent.com/JerBouma/MacroDatabase/master/"
                            "Database/JSON_Files.json/", 'r'))
...
def show_options(query):
    ...
    df = pd.DataFrame(columns=['Dataset', 'Main File', "Files"])
    ...
    for key in JSON_FILES:
        try:
            ...
            if query in string:
                df = pd.concat([df, pd.DataFrame([[key, f"_{key}.json", len(JSON_FILES[key])]])])
