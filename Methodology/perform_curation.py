...
COUNTRIES = json.load(open(f"{path_str}/_Structure/countries.json", "r"))
CURATION_LIST = json.load(open(f"{path_str}/_Structure/curation_list.json", "r"))
...
json_structure = json.load(open(f"{path_str}/_Structure/json_structure.json", "r"))
...
for key_1, values_1 in curated.items():
    if key_1 not in os.listdir(f"{path_str}/_Curated"):
        os.mkdir(f"{path_str}/_Curated/{key_1}")

    for key_2, values_2 in values_1.items():
        key_2 = key_2.replace("/", " ")

        if key_2 not in os.listdir(f"{path_str}/_Curated/{key_1}"):
            try:
                os.mkdir(f"{path_str}/_Curated/{key_1}/{key_2}")
        ...
        if len(keys) == 1:
            json.dump(values_2[keys[0]], open(f"{path_str}/_Curated/{key_1}/{key_2}/{key_2}.json", "w"), indent=2)
        else:
            for key_3, values_3 in values_2.items():
                json.dump(values_2[key_3], open(f"{path_str}/_Curated/{key_1}/{key_2}/{key_3}.json", "w"), indent=2)
